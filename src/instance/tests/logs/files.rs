...
fn create_log_file_name_has_millisecond_precision() {
    ...
    assert!(stem[stem.len() - 3..].chars().all(|c| c.is_ascii_digit()), "name={name}");

    // pin the pattern: %3f must be zero-padded so names stay sortable
    let dt = chrono::NaiveDateTime::parse_from_str(
        "2025-01-02T15:04:05.005",
        "%Y-%m-%dT%H:%M:%S%.3f",
    )
    .unwrap();
    assert_eq!(dt.format("%Y-%m-%d_%H-%M-%S%3f").to_string(), "2025-01-02_15-04-05005");
}
